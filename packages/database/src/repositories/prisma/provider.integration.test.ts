...
import { getPrismaClient, disconnectPrisma } from "@lufa/database/prisma";
import {
...
describe("PostgreSQL provider contract", () => {
  ...
  it("implements all seven domain repositories and preserves relationships", async () => {
    ...
    const rankings = await serviceContainer.rankingService.getRankings({
      mode: "count",
      eventType: "touchdown",
  ...
  it("updates an admin user by id even when it is outside the 300-row listing", async () => {
    ...
    const updated = await serviceContainer.adminService.updateUser(actor!, "integration-old-user", { role: "juez" });
