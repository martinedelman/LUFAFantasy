...
export interface FileStoragePort {
  create(input: { pathname: string; file: Blob; contentType?: string }): Promise<StoredFileMetadata>;
  read(url: string): Promise<StoredFileMetadata | null>;
  update(currentUrl: string, input: { pathname: string; file: Blob; contentType?: string }): Promise<StoredFileMetadata>;
...
export class BlobStorageService {
  ...
  constructor(
    private readonly fileStorageRepo: FileStoragePort,
    private readonly appEnvironment: () => "development" | "testing" | "production",
  ) {}

  private getEnvironmentFolder(): "prod_env" | "dev_env" {
    return this.appEnvironment() === "production" ? "prod_env" : "dev_env";
  }
