...
import { getAppEnvironment, hasConfiguredAppEnvironment } from "@lufa/database/appEnvironment";
import { getDatabaseProvider } from "@lufa/database";

export const dynamic = "force-dynamic";
export const runtime = "nodejs";

const SERVICE_NAME = "lufa-api";
...
function getConfigurationComponent(): PublicHealthComponentDto {
  const configured =
    hasConfiguredAppEnvironment() &&
    Boolean(process.env.JWT_SECRET) &&
    Boolean(process.env.INSTITUTIONAL_APP_URL || process.env.NEXT_PUBLIC_APP_URL);
...
async function getDatabaseComponent(): Promise<PublicHealthComponentDto> {
  ...
  if (provider === "postgres" ? !process.env.DATABASE_URL : !process.env.MONGODB_URI) {
    return {
      id: "database",
      status: "degraded",
      detail: `Database is not configured (${provider})`,
    };
  }

  try {
    const { getReportingRepository } = await import("@lufa/database/repositories/reporting");
    await getReportingRepository().checkDatabaseHealth();

    return {
      id: "database",
      status: "ok",
      detail: `Database is reachable (${provider})`,
    };
  } catch {
    return {
      id: "database",
      status: "degraded",
      detail: `Database is not available (${provider})`,
    };
  }
}
