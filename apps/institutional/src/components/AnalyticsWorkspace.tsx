...
function MultiFilter({ label, options, value, onChange }: { label: string; options: Array<{ value: string | number; label: string }>; value: Array<string | number>; onChange: (values: string[]) => void }) {
  return <label className="text-sm font-semibold">{label}<select multiple className="mt-1 block min-w-40 rounded-md border border-slate-300 p-2 text-sm" value={value.map(String)} onChange={(event) => onChange([...event.currentTarget.selectedOptions].map((option) => option.value))}>{options.map((item) => <option key={item.value} value={String(item.value)}>{item.label}</option>)}</select></label>;
}
...
function Widget({ widget, rows, editing, catalog, onRemove, onUpdate, onCrossFilter, onExport }: { widget: AnalyticsWidgetDto; rows: AnalyticsResultRowDto[]; editing: boolean; catalog: AnalyticsCatalogDto; onRemove: () => void; onUpdate: (patch: Partial<AnalyticsWidgetDto>) => void; onCrossFilter: (row: AnalyticsResultRowDto) => void; onExport: () => void }) {
  ...
  return <section className="group h-full overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm" aria-label={widget.title}>
    ...
    <div className="h-[calc(100%-61px)] p-4">
      ...
      {widget.visualization === "card" ? <div className="flex h-full flex-col justify-center"><p className="text-4xl font-black text-slate-950">{Number(cardValue.toFixed(2))}</p><p className="mt-2 text-sm text-slate-500">{rows.length ? "Resultado con los filtros activos" : "Sin datos en este alcance"}</p></div> : null}
      {widget.visualization === "table" ? <div className="h-full overflow-auto"><table className="min-w-full text-left text-sm"><thead className="sticky top-0 bg-white text-xs uppercase text-slate-500"><tr><th className="pb-2">Categoría</th><th className="pb-2 text-right">Valor</th></tr></thead><tbody>{rows.map((row) => <tr key={row.key} className="cursor-pointer border-t border-slate-100 hover:bg-brand-100/40" onClick={() => onCrossFilter(row)}><td className="py-2 font-medium text-slate-800">{row.label}</td><td className="py-2 text-right font-bold">{row.value}</td></tr>)}</tbody></table>{!rows.length ? <p className="py-8 text-center text-sm text-slate-500">Sin datos para estos filtros.</p> : null}</div> : null}
      {widget.visualization !== "card" && widget.visualization !== "table" ? <AnalyticsChart visualization={widget.visualization} rows={rows} onSelect={onCrossFilter} /> : null}
    </div>
  </section>;
}
...
export default function AnalyticsWorkspace() {
  ...
  const load = useCallback(async () => {
    try { setBusy(true); const [nextCatalog, nextReports] = await Promise.all([api<AnalyticsCatalogDto>("/api/analytics/catalog"), api<AnalyticsReportDto[]>("/api/analytics/reports")]); setCatalog(nextCatalog); setReports(nextReports); setReport((current) => current || nextReports[0] || null); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "No se pudo cargar analítica"); } finally { setBusy(false); }
  }, []);
  useEffect(() => { void load(); }, [load]);
  const effectiveFilters = useMemo(() => ({ ...filters, ...crossFilters }), [crossFilters, filters]);
  const refresh = useCallback(async () => {
    if (!report) return;
    try { setBusy(true); setError(null); setResult(await api<AnalyticsQueryResponseDto>("/api/analytics/query", { method: "POST", body: JSON.stringify({ widgets: report.widgets, filters: { ...report.filters, ...effectiveFilters } }) })); }
  ...
  return <main className="min-h-screen bg-slate-50"><div className="mx-auto max-w-[1600px] px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div ref={containerRef}>{mounted && report ? <Responsive width={width} layouts={layouts} breakpoints={{ lg: 1200, md: 900, sm: 640, xs: 0 }} cols={{ lg: 12, md: 8, sm: 4, xs: 1 }} rowHeight={30} margin={[16, 16]} dragConfig={{ enabled: editing, handle: ".widget-drag-handle" }} resizeConfig={{ enabled: editing }} onLayoutChange={(_layout: Layout, nextLayouts: ResponsiveLayouts) => updateReport({ layouts: nextLayouts as AnalyticsReportDto["layouts"] })}>{report.widgets.map((widget) => <div key={widget.id}><Widget widget={widget} rows={resultsByWidget.get(widget.id) || []} editing={editing} catalog={catalog} onRemove={() => updateReport({ widgets: report.widgets.filter((item) => item.id !== widget.id) })} onUpdate={(patch) => updateReport({ widgets: report.widgets.map((item) => item.id === widget.id ? { ...item, ...patch } : item) })} onCrossFilter={(row) => applyCrossFilter(widget, row)} onExport={() => void exportWidget(widget)} /></div>)}</Responsive> : <div className="grid min-h-60 place-items-center rounded-xl border border-dashed border-slate-300 bg-white text-sm text-slate-500">{busy ? "Cargando tablero…" : "No hay tablero disponible."}</div>}</div>
  </div></main>;
}
